...
def make_withdraw(balance, password):
    ...
    attempts = []
    time = 0
    def account(amount, input):
        nonlocal balance, time
        if time < 3:
            if input == password:
                if balance >= amount:
                    balance -= amount
...
def repeated(t, k):
    ...
    for x in t:
        if x == last:
            a += 1
            if a == k:
                return x      # why return ,but not yield?
        else:
            a = 1
            last = x
#for statement in a iterator means traversing it in order.
#call a generator function yields a generator, which means it will not
#return any value until being called by next method.
