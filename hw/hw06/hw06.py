...
class VendingMachine:
    ...
    def vend(self):
        if self.stock == 0:
            ...
            return 'Here is your {0}.'.format(self.kind)
        else :
            self.stock -= 1
            change = self.funds - self.price
            self.funds = 0
            return 'Here is your {0} and ${1} change.'.format(self.kind, change)
    ...
    def restock(self, num):
        self.stock += num
        return 'Current {0} stock: {1}'.format(self.kind, self.stock)
...
class Coin:
    ...
    def worth(self):
        "*** YOUR CODE HERE ***"
        value = max(0, Mint.current_year - self.year - 50) + self.cents
        return value
class Nickel(Coin):
    cents = 5
...
def is_bst(t):
    ...
    if t.is_leaf():
        ...
        if t.label >= bst_max(left) and t.label <= bst_min(right):
            return all([is_bst(left), is_bst(right)])
        else:
            return False
