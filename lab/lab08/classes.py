...
class TutorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        opponent.hand.extend([opponent.deck.draw() for i in range(3)])
        return
...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        "*** YOUR CODE HERE ***"
        copy1 = [player.deck.draw() for i in range(len(player.deck.cards))]
        copy2 = [opponent.deck.draw() for i in range(len(opponent.deck.cards))]
        
        for i in copy1:
            i.attack += other_card.attack
            i.defense += other_card.defense
            
        copy2 = [i for i in copy2 if i.attack != other_card.attack or i.defense !=other_card.defense]
        
        player.deck = Deck(copy1)
        opponent_card = Deck(copy2)
