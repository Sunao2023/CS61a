...
def eval_all(expressions, env):
    ...
    prt = expressions
    while prt.rest is not nil:
        prt = prt.rest
    return prt.first
    # END PROBLEM 7
'''Compared with 'and' and 'or' procedure, eval_all function evaluate all the result in the expression and then return the last one. If any error occers, raise an error. But in 'and' and 'or' procedure, not all of their sub-expressions may be evaluated.
'''
################
# Environments #
################
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        f, v = formals, vals #use a copy so that the inital formals and vals
        while f != nil:      # are not changed, which may result other problems
            child_frame.define(f.first, v.first)# in the rest of the project.
            f = f.rest
            v = v.rest
        return child_frame
        # END PROBLEM 10
...
'''Why use self.env but not env? Because LambdaProcedure is a lexically-scoped procedure, which means when calling a Lambda Function, the new-created frame should be a child of the frame in which the lambda is defined. That means the argument 'env' is not used.
But in MuProcedure, since the MuProcedure has no self-env, it create a child frame of the frame where it is called, that means it's a dynamic-scoped procedure.
'''
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        name = target.first
        formals = target.rest
        validate_formals(formals) # varify whether the formals are valid.
...
def do_and_form(expressions, env):
    ...
    if expressions == nil:
        return True
    prt = expressions
    while prt is not nil:
        value = scheme_eval(prt.first, env)
...
def do_or_form(expressions, env):
    ...
    return False
    # END PROBLEM 12
'''The structure of these two function is similiar, because they have the same logic. Both of them are short-circuting, but in a while loop, 'and' procedure should return false as soon as a false value occers. Finally it returns the last value if it jumps out of the loop. Instead, 'or' procedure returns the value as soon as a true value occers, and finally, it will return False out of the loop.
'''
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        validate_form(binding, 2, 2) #make sure every binding has two arguments
