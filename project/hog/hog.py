...
def roll_dice(num_rolls, dice=six_sided):
    ...
    for i in range(num_rolls):
        total += dice()
        if total == total_test:
            for i in range(num_rolls-count-1):
                dice()
            return 1
        total_test = total+1
        count += 1
...
def is_swap(player_score, opponent_score):
    ...
    "*** YOUR CODE HERE ***"
    # END PROBLEM 4
    diff = player_score % 100 % 10 - opponent_score % 100 %10
    if diff == opponent_score % 100 // 10 or -diff == opponent_score % 100 // 10:
        return True
    return False
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    "*** YOUR CODE HERE ***"
    # END PROBLEM 5
    score_add0 = score_add1 = bonus = turn =  0
    while score0 < goal and score1 < goal:
        ...
        score_add = take_turn(strategy(score, opponent), opponent, dice)
        
        if(feral_hogs):
            if abs(feral - strategy(score, opponent)) == 2:
                bonus = 3
            else:
                bonus = 0
        
        if who == 0 :
            score0 += score_add + bonus
            player_score = score0
            score_add0 = score_add
        else :
            score1 += score_add + bonus
            player_score = score1
            score_add1 = score_add
            
        if is_swap(player_score, opponent):
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    a = averaged_dice(1, dice)
    for i in range(2, 11):
        b = averaged_dice(i, dice)
        if b > a:
            max = i
        a = b
    return max
