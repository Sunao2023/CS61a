...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 9
            if isinstance(self, BodyguardAnt) or isinstance(self, TankAnt):
                if self.can_contain(place.ant):
                    self.contain_ant(place.ant)
                    place.ant = self
                else:
                    assert place.ant is None, 'Two ants in {0}'.format(place)
            elif isinstance(place.ant, BodyguardAnt) or isinstance(place.ant, TankAnt):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        def helper(place, beehive, count):
            if place is beehive:
                return None
            elif not place.bees or count < self.min_range or count > self.max_range:
                return helper(place.entrance, beehive, count + 1)
            else:
                return rANTdom_else_none(place.bees) # REPLACE THIS LINE
        return helper(self.place, beehive, 0)
        # END Problem 3 and 4
...
class ScubaThrower(ThrowerAnt):
    ...
    def __init__(self, armor = 1):
        return ThrowerAnt.__init__(self, armor)
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        def helper(place):
            ...
            if not place.ant:
                ...
                if isinstance(place.ant, TankAnt) or isinstance(place.ant, BodyguardAnt):
                    if place.ant.doubeled == False:
                        place.ant.damage *= 2
                        place.ant.doubeled = True
                    if place.ant.contained_ant and place.ant.contained_ant.doubeled == False:
                        place.ant.contained_ant.damage *= 2
                        place.ant.contained_ant.doubeled = True
                    return helper(place.exit)
                else:
                    if place.ant.doubeled == False:
                        place.ant.damage *= 2
