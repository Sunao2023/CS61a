...
def choose(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    chosen = [x for x in paragraphs if select(x)]
    if k >= len(chosen):
        return ''
    return chosen[k]
    # END PROBLEM 1
...
def accuracy(typed, reference):
    ...
    for i in typed_words:
        ...
        if i == reference_words[index]:
            matched += 1
        index += 1
    return 100*matched/len(typed_words)
    # END PROBLEM 3
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    # BEGIN PROBLEM 5
    "*** YOUR CODE HERE ***"
    if min([diff_function(user_word, x, limit) for x in valid_words]) > limit or user_word in valid_words:
        return user_word
    else:
